...
`)}
    ...
    </a>`}function g(o,t){let s="";t.innerHTML="";for(const n of o)s+=h(n);t.insertAdjacentHTML("beforeend",s)}const i=document.querySelector("form"),c=document.querySelector("main"),l=document.querySelector("#loader");i.addEventListener("submit",async o=>{o.preventDefault();const t=i.elements.image.value;if(!t.trim())return;l.style.display="";const s=await p(t);if(l.style.display="none",s.length==0)return c.innerHTML="",f.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"});g(s,c),new m("main a",{captionsData:"alt",captionDelay:250}).refresh()});
